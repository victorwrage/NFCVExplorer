 ...
 * @date 2013-07-12
 */
public class ActivityTagSave extends BaseActivity {
	...
	protected void onNewIntent(Intent intent) {
		...
		if (NFCUtils.writeTag(intent,
			...
			NFCUtils.toast(ActivityTagSave.this.getApplicationContext(), "写入失败，但仍然可以工作，需要选择本应用在弹出的窗口");
		}
	}
	...
	public void onResume() {
		...
		try {
			if (adapter_nfc != null) {
				adapter_nfc.enableForegroundDispatch(ActivityTagSave.this,
	...
	protected void onPause() {
		super.onPause();
		if (adapter_nfc != null) {
			adapter_nfc.disableForegroundDispatch(ActivityTagSave.this);
		}
	}
	/**
	 * 保存TAG

...
public class WriteTagActivity extends BaseActivity {
	...
	public void onResume() {
		...
		try {
			if (adapter_nfc != null) {
				adapter_nfc.enableForegroundDispatch(WriteTagActivity.this,
	...
	protected void onNewIntent(Intent intent) {
		...
		if (NFCUtils.writeTagByNdefMessage(intent, message)) {
			NFCUtils.toast(WriteTagActivity.this.getApplicationContext(), "写入成功");
			LogUtil.v("WriteTagActivity---success");
			finish();
		}else{
			NFCUtils.toast(WriteTagActivity.this.getApplicationContext(), "写入失败");
		}
	}
	...
	protected void onPause() {
		...
		super.onPause();
		if (adapter_nfc != null) {
			adapter_nfc.disableForegroundDispatch(WriteTagActivity.this);
		}
	}
}

 ...
 * 
 */
public class ActivitySwitchSave extends BaseActivity {
	...
	protected void onNewIntent(Intent intent) {
		...
		if(NFCUtils.writeTag(intent, "nfcv://tags.do/"+Constants.current_card.getCard_uid())){
			processCard();
			NFCUtils.toast(ActivitySwitchSave.this.getApplicationContext(), "写入成功");
		}else{
			NFCUtils.toast(ActivitySwitchSave.this.getApplicationContext(), "写入失败，但仍然可以工作，需要选择本应用在弹出的窗口");
		}
	}
	@Override
	protected void onPause() {
		...
		adapter_nfc.disableForegroundDispatch(ActivitySwitchSave.this);
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case 0:
			if (!isScaned) {
				NFCUtils.toast(ActivitySwitchSave.this, "未扫描到标签");
				return true;
			}
			saveTag();
